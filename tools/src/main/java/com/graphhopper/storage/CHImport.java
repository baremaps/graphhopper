...
public class CHImport {
    ...
    private static void own(String[] args) throws IOException {
        ...
        List<CHConfig> chConfigs = profiles.stream().map(p ->
                CHConfig.edgeBased(p.getName(), new FastestWeighting(em.getEncoder(p.getName())))).collect(Collectors.toList());
        GraphBuilder graphBuilder = new GraphBuilder(em);
        graphBuilder.setCHConfigs(chConfigs);
